...
export const register = async (req, res) => {
    const idUser = uuid();
    const { username, password, fullname, gender, email, phone, idKamar, tanggalMasuk, isAdmin } = req.body;
    try {
        if (!username || !password || !fullname || !gender || !email || !phone || !idKamar || !tanggalMasuk) {
...
export const Logout = async (req, res) => {
    try {
        return res.status(200).clearCookie("token").json({ success: true, msg: "Logout Berhasil!" });
    } catch (error) {
